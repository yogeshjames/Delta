...
function Card({ song, func, is, playSong ,inc,index,priv}) {///each has differnt using priv for private card
  ...
  useEffect(() => {
    const fetchPlaylists = async () => {
      try {
        const res = await axios.get('http://localhost:3000/playlistdetails',{params:{id:window.localStorage.getItem('id')}});
    ...
    <div className="flex m-7">
      <div
        onClick={() => {playSong(song)
          ...
          if(inc){// check if i pass function that i sonly if its from playlist compoentn do this
            console.log(index)
          inc(++index)}
        }}
        className="card bg-slate-100 cursor-pointer button-hover-effect shadow-md rounded-md p-4 m-2 w-60"
      >
        <img src={priv ? song.image : song.album.images[0].url } alt={song.name} className="w-full h-40 object-cover rounded-md" />
        <div className="mt-4">
          <h2 className="text-xl font-semibold">{song.name}</h2>
          <p className="text-gray-600">{ priv ?  song.artist :  song.artists[0].name}</p>
        </div>
        {is && (/// in above cant use || or coz it gives errro ehilw trying to acces song.artists which is not present if its priv soo use this
